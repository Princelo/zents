...
class CircleCrop
{
    ...
    public function __construct($img, $strPath)
    {
        $this->src_img = $img;
    ...
    public function save(){
        $save_path = substr($this->strPath, 0, strpos($this->strPath, ".")) . "_circle.png";
        $result = imagepng($this->dst_img, $save_path, 9);
        if($result === true){
            return true;
        }else{
            return false;
    ...
    public function strGetSavePath(){
        return substr($this->strPath, 0, strpos($this->strPath, ".")) . "_circle.png";
    }
