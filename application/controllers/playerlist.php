...
class Playerlist extends CI_Controller {
    ...
    public function index($offset = 0)
    {
        if(isset($_POST['search']) && trim($_POST['search']) != ""){
            ...
            $data['playerlist'] = $this->MPlayer->objGetPlayers(" order by sort desc, id ".$limit, $where);
            ...
            $this->load->view('playerlist/index', $data);
        ...
        $data['playerlist'] = $this->MPlayer->objGetPlayers(" order by sort desc, id ".$limit);
        $expiration = time() - 3600 * 24;
        $data['votedlist'] = $this->MPlayer->arrGetVotedPlayers($_SERVER['REMOTE_ADDR'], $expiration);
